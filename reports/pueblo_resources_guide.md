# Pueblo Resources Guide

Community safety in Pueblo depends on quick access to trusted legal, housing, health, and survivor services. Use this guide to match urgent needs with organizations already operating in the county, and pair every outreach with documentation (names, dates, case IDs) so follow-up teams can advocate effectively.

_Last refreshed (map sync): 2026-02-19_

## Executive Summary
- Civil legal aid: **Colorado Legal Services (CLS)** runs the Southeastern Colorado office in downtown Pueblo, handling eviction defense, protection orders, and limited immigration screenings for income-qualified residents.[^cls]
- Housing & homelessness: **Posada** coordinates motel vouchers, mail services, and supportive housing; partner site **SafeSide Recovery** doubled shelter capacity with eight new dorm-style units in Jan 2026.[^posada][^safeside]
- Survivor safety: **YWCA Pueblo** (domestic violence) and **Rape Crisis Services** offer 24/7 hotlines, shelter/accompaniment, and bilingual advocates.[^ywca][^rcs]
- Mental health: **Health Solutions’ Crisis Walk-In Center** provides no-cost stabilization, mobile crisis, and detox linkages regardless of insurance.[^hs]

## Quick Reference Table
| Need | Organization | Contact | How they help |
| --- | --- | --- | --- |
| Eviction, civil legal aid, VAWA filings | Colorado Legal Services — Pueblo Office | 719-545-6708; 1000 W. 6th St., Suite I | Represents low-income clients on housing, DV, consumer, and limited immigration matters; bilingual intake. |
| Homelessness, mail service, supportive housing | Posada | 719-545-8776; 1008 N. Grand Ave. | Provides motel vouchers, case management, mailboxes, and permanent supportive housing placements for families, veterans, and youth. |
| 24/7 low-barrier shelter + recovery support | SafeSide Recovery (Posada partner) | 719-696-0996; 901 W. 8th St. | Offers round-the-clock shelter; Jan 2026 expansion added eight dorm-style units, hygiene services, and recovery-oriented case management. |
| Domestic violence crisis + shelter | YWCA Pueblo | 24/7 hotline 719-545-4884 | Operates emergency shelter, legal advocacy, and economic empowerment programming for survivors and dependents. |
| Sexual assault crisis response | Rape Crisis Services of Pueblo | 24/7 hotline 719-544-1191 | Sends advocates to hospitals/law enforcement, offers counseling referrals, and facilitates prevention education. |
| Behavioral health & substance-use crisis | Health Solutions Crisis Walk-In Center | 844-493-8255; 711 W. 13th St. | 24/7 stabilization, mobile crisis response, peer support, and detox coordination for youth and adults. |
| Live ICE sightings & documentation | Colorado Rapid Response Network | Hotline 844-864-8341 (opt.1 live / opt.2 past) | Dispatches trained observers statewide, logs incidents, and links callers to immigrant-rights coalitions.[^corrn] |
| Detention legal aid | Rocky Mountain Immigrant Advocacy Network (RMIAN) | 303-433-2812 (adults) / 303-222-0003 (children) | Provides free legal orientation/representation for detained adults and unaccompanied kids; offers social-service coordination.[^rmian] |
| Statewide crisis counseling | Colorado Crisis Services | Hotline 844-493-8255; text TALK to 38255 | 24/7 statewide counseling + mobile response coordination; connects callers to local walk-in centers.[^ccs] |
| Childcare & family services | Catholic Charities of Pueblo | 719-544-4233; 429 W. 10th St. | Early Head Start/childcare slots, parenting classes, and emergency assistance case management.[^ccp] |
| Tech/device access | Pueblo Library TechConnect | 719-562-5600; 100 E. Abriendo Ave. | Loans Wi-Fi hotspots/Chromebooks, offers printing and digital literacy help.[^techconnect] |
| Mutual aid & basic needs | Pueblo Cooperative Care Center | 719-543-7484; 326 W. 8th St. | Provides food, clothing, hygiene kits, and mail services for residents in crisis.[^coopcare] |

## Self-Advocacy Moves
1. **Document eligibility on intake.** Capture names, date/time, staff contact, and any case/reference numbers the organization provides so future volunteers can check status without repeating the story.
2. **Pair legal + social services.** When CLS opens an eviction or DV case, loop in Posada or YWCA for housing safety planning so legal timelines don’t leave families without shelter.
3. **Hotline call scripts.** Before dialing YWCA or Rape Crisis Services, write down a 3-line script (who needs help, immediate safety risks, callback number) to move through triage faster.
4. **Mental health + protest support.** For demonstrations, stage a Health Solutions crisis-line card at the medic table so participants know where to seek 24/7 stabilization if trauma surfaces later.
5. **Track referrals centrally.** Update the Obsidian vault or encrypted spreadsheet with every referral (date, org, outcome) to measure gaps and escalate to funders or officials when capacity runs out.

## Source Notes
- [^cls]: Colorado Legal Services — Pueblo Office, https://www.coloradolegalservices.org/node/10/pueblo-office (accessed 2025-11-15).
- [^posada]: Posada Pueblo Services, https://posadapueblo.org/services/ (accessed 2025-11-15).
- [^ywca]: YWCA Pueblo Domestic Violence Services, https://www.ywcapueblo.org/our-work/domestic-violence-services/ (accessed 2025-11-15).
- [^rcs]: Rape Crisis Services of Pueblo, https://rapecrisisservices.org/ (accessed 2025-11-15).
- [^hs]: Health Solutions Crisis Care, https://www.health.solutions/services/crisis-care/ (accessed 2025-11-15).
- [^corrn]: Colorado Rapid Response Network, https://www.coloradorapidresponsenetwork.com/ (accessed 2025-11-15).
- [^rmian]: Rocky Mountain Immigrant Advocacy Network – Get Help, https://rmian.org/get-help/ (accessed 2025-11-15).
- [^ccs]: Colorado Crisis Services, https://coloradocrisisservices.org/ (accessed 2025-11-15).
- [^ccp]: Catholic Charities of the Diocese of Pueblo, https://www.ccpueblo.org/services/ (accessed 2025-11-15).
- [^techconnect]: Pueblo City-County Library District TechConnect, https://www.pueblolibrary.org/techconnect (accessed 2025-11-15).
- [^coopcare]: Pueblo Cooperative Care Center, https://cooperativecarecenter.org/ (accessed 2025-11-15).
- [^safeside]: KOAA News5, “Pueblo homeless facility expanding to help more people,” Jan 9, 2026.
