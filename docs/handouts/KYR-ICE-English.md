# KNOW YOUR RIGHTS: If ICE Comes to Your Door

**PUEBLO, CO - Updated November 2025**

---

## YOU HAVE THE RIGHT TO REMAIN SILENT

---

### IF SOMEONE KNOCKS ON YOUR DOOR

1. **DO NOT OPEN THE DOOR**
   - You have the right to keep your door closed
   - Speak through the door or window

2. **ASK: "Do you have a warrant signed by a judge?"**
   - If NO → Say: "I choose not to open the door"
   - If YES → Ask them to slide it under the door
   - A real warrant has a judge's signature and your correct address

3. **DO NOT SIGN ANYTHING**
   - Without talking to a lawyer first

4. **STAY CALM AND SILENT**
   - You have the right to remain silent
   - Say: "I am exercising my right to remain silent"

---

## IMPORTANT: NOVEMBER 25, 2025 COURT RULING

**Case: *Ramirez Ovando v. Noem* — U.S. District Court, Colorado**

**A federal judge ruled ICE warrantless arrests are UNLAWFUL in Colorado.**

ICE must now prove you are a flight risk BEFORE arresting without a warrant.

If ICE tries to arrest you without a warrant, say:
> "Under *Ramirez Ovando v. Noem*, ICE cannot make warrantless arrests in Colorado without proving flight risk."

**If arrested after Nov 25, 2025 without warrant:** You may be eligible for bond refund and ankle monitor removal. Contact ACLU of Colorado immediately.

---

## SHERIFF LUCERO'S PLEDGE (January 2025)

Pueblo County Sheriff WILL NOT:
- Participate in ICE roundup operations
- Enforce based solely on immigration status
- Help ICE at schools, churches, or hospitals

---

## EMERGENCY NUMBERS - SAVE THESE NOW

| **CORRN Hotline (ICE emergencies)** | **844-864-8341** |
|-------------------------------------|------------------|
| RMIAN Detention Hotline | (303) 866-9308 |
| Catholic Charities Immigration | (719) 586-8600 |
| Colorado Legal Services | 719-545-6708 |

---

## IF A FAMILY MEMBER IS DETAINED

1. **Don't panic** - They have rights
2. **Call CORRN:** 844-864-8341 (press 1)
3. **Call RMIAN:** (303) 866-9308
4. **Have ready:** Full name, date of birth, A-number if known, detention location

---

## YOUR CONSTITUTIONAL RIGHTS

✓ **5th Amendment:** Right to remain silent
✓ **4th Amendment:** Right against unreasonable searches
✓ **Right to an attorney** before answering questions

**These rights apply to EVERYONE regardless of immigration status**

---

## QUICK SAFETY TIPS

- [ ] Remove location from social media posts
- [ ] Use Signal instead of WhatsApp
- [ ] Don't discuss immigration status on social media
- [ ] Create a family emergency plan
- [ ] Save emergency numbers in your phone

---

**CORRN HOTLINE: 844-864-8341**

*Print this. Carry it. Share it.*

---

*Part of La Raza VII.I.IX - Community Field Guide for Pueblo, CO*
*github.com/samedayhurt/laraza*
